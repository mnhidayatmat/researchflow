    ...
    <div x-data="ganttChartApp({{ $student->id }})" x-cloak class="space-y-4">
        ...
        <div class="bg-card rounded-2xl border border-border p-4">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <div class="flex items-center gap-0.5 bg-surface rounded-lg p-0.5 border border-border-light">
                        <button @click="navigate('prev')" class="p-1.5 rounded-md text-secondary hover:text-primary hover:bg-card transition-all">
                        ...
                        <button @click="navigate('next')" class="p-1.5 rounded-md text-secondary hover:text-primary hover:bg-card transition-all">
                    ...
                    <div class="flex items-center gap-0.5 bg-surface rounded-lg p-0.5 border border-border-light">
                        <button @click="zoom('out')" class="p-1.5 rounded-md text-secondary hover:text-primary hover:bg-card transition-all">
                        ...
                        <button @click="zoom('in')" class="p-1.5 rounded-md text-secondary hover:text-primary hover:bg-card transition-all">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <div class="hidden sm:flex items-center gap-2">
                        <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-success/10 text-success text-xs font-medium">
                            <span x-text="taskStats.completed || 0">0</span> Done
                        </div>
                        <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-warning/10 text-warning text-xs font-medium">
                            <span x-text="taskStats.inProgress || 0">0</span> Active
                        </div>
                    </div>

                    <button @click="refresh()" class="p-2 text-secondary hover:text-primary hover:bg-surface rounded-xl transition-all">
        ...
        <div x-show="loading" class="flex flex-col items-center justify-center py-20">
            <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-accent/20 to-accent/5 flex items-center justify-center">
                <svg class="w-6 h-6 text-accent animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
            </div>
        ...
        <div x-show="!loading" class="bg-card rounded-2xl border border-border overflow-hidden shadow-soft">
            ...
            <div class="flex border-b border-border bg-surface/50">
                <div class="flex-shrink-0 w-[260px] px-4 py-3 border-r border-border">
                    <span class="text-xs font-semibold text-secondary uppercase tracking-wider">Task</span>
                </div>
            ...
            <div class="flex" style="max-height: 600px;">
                ...
                <div class="flex-shrink-0 w-[260px] border-r border-border bg-card overflow-y-auto">
                    <div class="divide-y divide-border">
                        <template x-for="(task, index) in tasks" :key="task.id">
                            <a :href="`/students/${studentId}/tasks/${task.task_id}`"
                               class="flex items-center gap-3 p-4 hover:bg-surface transition-colors"
        ...
        <div x-show="!loading && tasks.length > 0" class="flex items-center justify-center gap-6 px-2 py-2">
            ...
            <span class="flex items-center gap-1.5 text-xs text-secondary">
                <svg class="w-4 h-4 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122"/></svg>
            ...
            window.ganttChartApp = function(studentId) {
                return {
                    ...
                    renderGantt() {
                        ...
                        this.ganttInstance = new Gantt(container, this.tasks, {
                            ...
                            custom_popup_html: (task) => {
                                ...
                                return '<div class="bg-white border border-gray-200 rounded-lg shadow-lg p-4 min-w-[220px]">' +
                                    '<div class="flex items-center justify-between mb-3">' +
                                    '<span class="font-semibold text-sm">' + task.name + '</span>' +
                                    '<span class="text-[10px] px-2 py-0.5 rounded-full" style="background-color: ' + statusColor + '20; color: ' + statusColor + '">' + statusLabel + '</span>' +
                                    '</div>' +
                                    ...
                                    '</div>' +
                                    '</div>' +
                                    '<a href="/students/' + this.studentId + '/tasks/' + taskId + '" class="block w-full text-center text-xs font-medium text-accent hover:text-amber-700 py-2 rounded-lg bg-accent/10 hover:bg-accent/20 transition-colors">View Details →</a>' +
                                    '</div>';
                            }
                        });
                    ...
                    applyCustomStyles() {
                        ...
                        styleEl.textContent =
                            '.gantt .bar { fill: #D97706; cursor: pointer; }\n' +
