...
<div class="overflow-hidden rounded-xl border border-border bg-white" x-data="{ selected: [] }">
    <div class="overflow-x-auto scrollbar-thin">
        <table class="w-full text-sm text-left">
            @if($headers)
                <thead class="text-xs text-secondary uppercase tracking-wider bg-surface/50 border-b border-border">
                    <tr>
            ...
            <tbody class="divide-y divide-border {{ $striped ? 'divide-y-0' : '' }}">
                @if($slot->isNotEmpty())
                    {{ $slot }}
                @else
                    <tr>
