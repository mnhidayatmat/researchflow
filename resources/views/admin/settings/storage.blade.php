<x-layouts.app title="Storage Settings">
    ...
    <div class="max-w-4xl" x-data="storageSettings('{{ $currentDisk }}')" x-init="init()">
        ...
        <div class="mb-6">
            ...
            <p class="text-xs text-secondary mt-0.5">Configure where uploaded files, documents, and research data are stored</p>
        </div>
        ...
        <div class="mb-4">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h2 class="text-base font-semibold text-primary">Storage Overview</h2>
                    <p class="text-xs text-secondary mt-0.5">Current usage and storage statistics</p>
                </div>
            </div>

            <div x-show="loadingStats" x-cloak class="bg-card rounded-2xl border border-border p-8 text-center">
                <div class="flex items-center justify-center gap-3">
                    ...
                    <span class="text-sm text-secondary">Loading storage statistics...</span>
                </div>
            </div>
            ...
            <div x-show="!loadingStats" x-cloak class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                ...
                <div class="group relative bg-card rounded-2xl p-6 border border-border hover:border-accent/30 hover:shadow-soft transition-all duration-300">
                    ...
                    <div class="mt-4">
                        <p class="text-lg font-bold text-primary capitalize">{{ str_replace('_', ' ', $currentDisk) }}</p>
                        <p class="text-xs text-secondary">Active Storage</p>
                    </div>
                </div>
                ...
                <template x-if="stats.file_count !== undefined">
                    <div class="group relative bg-card rounded-2xl p-6 border border-border hover:border-info/30 hover:shadow-soft transition-all duration-300">
                        ...
                        <div class="mt-4">
                            <p class="text-lg font-bold text-primary" x-text="stats.file_count?.toLocaleString() || '0'"></p>
                            <p class="text-xs text-secondary">Total Files</p>
                        </div>
                    </div>
                </template>
                ...
                <template x-if="stats.total_size_human !== undefined">
                    <div class="group relative bg-card rounded-2xl p-6 border border-border hover:border-success/30 hover:shadow-soft transition-all duration-300">
                        ...
                        <div class="mt-4">
                            <p class="text-lg font-bold text-primary" x-text="stats.total_size_human || '0 B'"></p>
                            <p class="text-xs text-secondary">Total Size</p>
                        </div>
                    </div>
                </template>
                ...
                <template x-if="stats.usage !== undefined">
                    <div class="group relative bg-card rounded-2xl p-6 border border-border hover:border-warning/30 hover:shadow-soft transition-all duration-300">
                        ...
                        <div class="mt-4">
                            <p class="text-lg font-bold text-primary" x-text="formatBytes(stats.usage)"></p>
                            <p class="text-xs text-secondary">Drive Usage</p>
                        </div>
                    </div>
                </template>
                ...
                <template x-if="stats.limit !== undefined">
                    <div class="group relative bg-card rounded-2xl p-6 border border-border hover:border-accent/30 hover:shadow-soft transition-all duration-300">
                        ...
                        <div class="mt-4">
                            <p class="text-lg font-bold text-primary" x-text="formatBytes(stats.limit)"></p>
                            <p class="text-xs text-secondary">Drive Limit</p>
                        </div>
                    </div>
                </template>
            </div>
        ...
        <div class="mb-4">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h2 class="text-base font-semibold text-primary">Select Storage Driver</h2>
                    <p class="text-xs text-secondary mt-0.5">Choose where your files will be stored</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <label
                    ...
                    :class="activeDisk === 'local' ? 'border-accent bg-accent/5 shadow-soft' : 'border-border hover:border-accent/30 bg-card hover:shadow-soft'"
                >
                    ...
                    <div class="w-14 h-14 rounded-2xl mb-4 flex items-center justify-center transition-transform duration-300"
                         :class="activeDisk === 'local' ? 'bg-gradient-to-br from-accent/20 to-accent/10 scale-110' : 'bg-surface group-hover:bg-accent/10'">
                    ...
                    <p class="text-xs text-secondary mt-1">Store files on server filesystem</p>
                ...
                <label
                    class="group relative flex flex-col p-6 rounded-2xl border-2 cursor-pointer transition-all duration-300"
                    :class="activeDisk === 'do_spaces' ? 'border-accent bg-accent/5 shadow-soft' : 'border-border hover:border-accent/30 bg-card hover:shadow-soft'"
                >
                    <input type="radio" name="disk_selector" value="do_spaces" x-model="activeDisk" class="sr-only" @change="onDiskChange()">
                    <div class="w-14 h-14 rounded-2xl mb-4 flex items-center justify-center transition-transform duration-300"
                         :class="activeDisk === 'do_spaces' ? 'bg-gradient-to-br from-accent/20 to-accent/10 scale-110' : 'bg-surface group-hover:bg-accent/10'">
                    ...
                    <p class="text-xs text-secondary mt-1">Cloud storage by DigitalOcean</p>
                ...
                <label
                    class="group relative flex flex-col p-6 rounded-2xl border-2 cursor-pointer transition-all duration-300"
                    :class="activeDisk === 'google_drive' ? 'border-accent bg-accent/5 shadow-soft' : 'border-border hover:border-accent/30 bg-card hover:shadow-soft'"
                >
                    <input type="radio" name="disk_selector" value="google_drive" x-model="activeDisk" class="sr-only" @change="onDiskChange()">
                    <div class="w-14 h-14 rounded-2xl mb-4 flex items-center justify-center transition-transform duration-300"
                         :class="activeDisk === 'google_drive' ? 'bg-gradient-to-br from-accent/20 to-accent/10 scale-110' : 'bg-surface group-hover:bg-accent/10'">
                    ...
                    <p class="text-xs text-secondary mt-1">Store files in Google Drive</p>
        ...
        <form method="POST" action="{{ route('admin.settings.storage.update') }}" class="mb-6">
            ...
            <div x-show="activeDisk === 'local'" x-cloak>
                <div class="bg-card rounded-2xl border border-border p-8">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <h3 class="text-base font-semibold text-primary">Local Disk Storage</h3>
                            <p class="text-sm text-secondary mt-1">Files are stored on the server's local filesystem at <code class="bg-surface px-2 py-0.5 rounded text-xs font-mono">storage/app/private</code></p>
                ...
                <div class="bg-card rounded-2xl border border-border overflow-hidden">
                    <div class="border-b border-border px-8 py-5">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="text-base font-semibold text-primary">DigitalOcean Spaces Configuration</h3>
                                <p class="text-xs text-secondary">Enter your DigitalOcean Spaces credentials</p>
                            </div>
                        </div>
                    </div>
                    <div class="p-8">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-xs font-medium text-secondary mb-1.5">Access Key ID</label>
                                <input type="text" name="do_spaces_key"
                                       value="{{ old('do_spaces_key', $settings['do_spaces_key'] ?? '') }}"
                                       placeholder="Your DO Spaces access key"
                                       class="w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm text-primary placeholder-secondary/50 focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none transition-all">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-secondary mb-1.5">Secret Access Key</label>
                            ...
                            <div>
                                <label class="block text-xs font-medium text-secondary mb-1.5">Bucket / Space Name</label>
                            ...
                            <div>
                                <label class="block text-xs font-medium text-secondary mb-1.5">Region</label>
                                <select name="do_spaces_region" class="w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm text-primary focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none transition-all">
                            ...
                            <div class="sm:col-span-2">
                                <label class="block text-xs font-medium text-secondary mb-1.5">CDN Endpoint (Optional)</label>
                                <input type="text" name="do_spaces_endpoint"
                                       ...
                                       class="w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm text-primary placeholder-secondary/50 focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none transition-all">
                                <p class="text-xs text-secondary mt-1.5">Leave blank to use the default Spaces endpoint</p>
                            </div>
                        </div>
                ...
                <div class="bg-card rounded-2xl border border-border overflow-hidden">
                    <div class="border-b border-border px-8 py-5">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="text-base font-semibold text-primary">Google Drive Integration</h3>
                                <p class="text-xs text-secondary">Configure OAuth 2.0 credentials for Google Drive API</p>
                            </div>
                        </div>
                    </div>
                    <div class="p-8">
                        <div class="space-y-6">
                            <div>
                                <label class="block text-xs font-medium text-secondary mb-1.5">Client ID</label>
                            ...
                            <div>
                                <label class="block text-xs font-medium text-secondary mb-1.5">Client Secret</label>
                            ...
                            <div>
                                <label class="block text-xs font-medium text-secondary mb-1.5">Refresh Token</label>
                            ...
                            <div>
                                <label class="block text-xs font-medium text-secondary mb-1.5">Folder ID (Optional)</label>
                                <input type="text" name="google_drive_folder_id"
                                       ...
                                       class="w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm text-primary placeholder-secondary/50 focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none transition-all">
                                <p class="text-xs text-secondary mt-1.5">Specify a folder ID to organize uploads in a specific folder</p>
                            </div>
                        </div>
                        <div class="mt-6 p-4 bg-info-light/30 border border-info-light rounded-xl">
                            <p class="text-xs text-primary font-medium mb-3">How to get credentials:</p>
                            <ol class="text-xs text-secondary space-y-2">
                                ...
                                <li class="flex items-start gap-2">
                                    ...
                                    <span>Use scope: <code class="bg-surface px-2 py-0.5 rounded text-xs font-mono">https://www.googleapis.com/auth/drive</code></span>
                                </li>
                            </ol>
                        </div>
                    </div>
            ...
            {{-- Actions --}}
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-6 bg-card rounded-2xl border border-border">
                ...
                <div x-data="testConnection()" x-show="activeDisk !== 'local'" class="flex-1">
                    <button
                        ...
                        @click="test()"
                        :disabled="testing"
                        class="inline-flex items-center gap-2.5 px-5 py-3 text-sm font-medium text-secondary border border-border rounded-xl hover:bg-surface transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                    ...
                    <div x-show="result" x-cloak class="mt-3">
                        ...
                        <p x-show="success" class="text-xs text-secondary mt-2 ml-1">
                            Tested disk: <span class="font-mono text-primary" x-text="testedDisk"></span>
                        </p>
                    </div>
                </div>
                <div x-show="activeDisk === 'local'" class="flex-1">
                    <p class="text-xs text-secondary">Local storage does not require connection testing.</p>
                </div>
